...
var handleSign = function handleSign(data) {
  console.log(data);
  var xhttp = new XMLHttpRequest();
  var postData = new FormData();
  xhttp.onreadystatechange = function () {
...
var init = function init() {
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      var response = JSON.parse(this.response);
      var req = response.registerRequests[0];
      var sigs = response.registeredKeys;
      if (sigs.length === 0) {
        var registerRequests = [{ version: req.version, challenge: req.challenge, attestation: 'direct' }];
        u2f.register(sigs, registerRequests, [], function (data) {
      ...
      if (sigs.length > 0) {
        var authhttp = new XMLHttpRequest();
        authhttp.onreadystatechange = function () {
          if (this.readyState == 4 && this.status == 200) {
            var _response = JSON.parse(this.response);
            var _req = _response;
            var appId = _req.appId;
            var signRequest = _response.challenge;
            u2f.sign([], signRequest, sigs, function (data) {
              return handleSign(data);
            });
          }
        };
        authhttp.overrideMimeType('application/json');
        authhttp.open("GET", "https://192.168.33.5/u2f/authenticaterequest", true);
        authhttp.send();
      }
    }
  };
  xhttp.overrideMimeType('application/json');
  xhttp.open("GET", "https://192.168.33.5/u2f/signrequest", true);
  xhttp.send();
};
